...
thetax2min=thetatd-dtheta/2*pi/180;     %single hemisphere nominal min/max theta
thetax2max=thetatd+dtheta/2*pi/180;
if thetatd < pi/2   %NH
  pmax=(Re+altmin)/Re/sin(thetax2min)^2;	         %bottom left grid point p, max p -> max theta
  ...
  pmax=(Re+altmin)/Re/sin(thetax2max)^2;	         %bottom left grid point p, max p -> min theta
  qtmp=(Re/(Re+altmin))^2*cos(thetax2max);	         %bottom left grid q (also bottom right)
  pmin=sqrt(cos(thetax2min)/sin(thetax2min)^4/qtmp); %bottom right grid p, why mixing of max/min here???
end
...
%coeffs=[3.3986e-04, 0.004341, -6.7597e-04]; % OPEN
coeffs=[3.3986e-04, 0.004341, -6.7597e-04]; % CLOSED
p(1)=pmin;
ip=1;
while p(ip)<pmax
...
% qmin/qmax need to be assigned based on hemisphere of source location
...
else                %close dipole grid; set max possible theta value for grid
   if thetatd < pi/2 %NH
...
if gridflag==0
    ...
    if (thetatd<pi/2)    % NH needs to be high-res.
        ...
        while q(iq)<=qmax
            iq=iq+1;
            dq=mindq+ampNH*(1/2-1/2*tanh((q(iq-1)-(qmax-qloc))/sigq));
            q(iq)=q(iq-1)+dq;
        end
        iq=1;
        q2(iq)=q(1)-(q(2)-q(1));
        ...
        while q2(iq)>=-qmax
            iq=iq+1;
            dq=(q(2)-q(1))-ampSH*(1/2-1/2*tanh((q2(iq-1)+(qmax-qloc))/sigq));
            q2(iq)=q2(iq-1)-dq;
        end
        q2=fliplr(q2);
        q=[q2,q];
    else                 % SH needs to be high-res.
        q(iq)=0;
        ampSH=amp;
        while q(iq)<=qmax
            iq=iq+1;
            dq=mindq+ampSH*(1/2-1/2*tanh((q(iq-1)-(qmax-qloc))/sigq));
            q(iq)=q(iq-1)+dq;
        end
        iq=1;
        q2(iq)=q(1)-(q(2)-q(1));
        ...
        while q2(iq)>=-qmax
            iq=iq+1;
            dq=(q(2)-q(1))-ampNH*(1/2-1/2*tanh((q2(iq-1)+(qmax-qloc))/sigq));
            q2(iq)=q2(iq-1)-dq;
        end
        q=-1*[fliplr(q),q2];
    end
...
xgf.I=reshape(xgf.I(1,inds2,inds3),[numel(inds2),numel(inds3)]);

xgf.nullpts=double(xgf.nullpts(inds1,inds2,inds3));
