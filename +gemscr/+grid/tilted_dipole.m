...
fprintf('\nMAKEGRID_DIPOLE.M --> Setting up q,p grid of size %d x %d.',cfg.lq,cfg.lp);
Re=6370e3;
...
rmin=p(cfg.lp)*Re*sin(thetax2min)^2; %use last field line to get qmin and qmax
rmax=p(cfg.lp)*Re*sin(thetamax)^2;
...
r=zeros(cfg.lq,cfg.lp);
fval=zeros(cfg.lp+1,cfg.lq+1);
theta=zeros(cfg.lq,cfg.lp);
qtol=1e-9;
...
r=repmat(r,[1 1  cfg.lphi]);
theta=repmat(theta,[1 1  cfg.lphi]);
...
qi=zeros(cfg.lq+1,1);
qi(2:cfg.lq)=1/2*(q(1:cfg.lq-1)+q(2:cfg.lq));
qi(1)=q(1)-1/2*(q(2)-q(1));
qi(cfg.lq+1)=q(cfg.lq)+1/2*(q(cfg.lq)-q(cfg.lq-1));
pii=zeros(1,cfg.lp+1);
pii(2:cfg.lp)=1/2*(p(1:cfg.lp-1)+p(2:cfg.lp));
pii(1)=p(1)-1/2*(p(2)-p(1));
pii(cfg.lp+1)=p(cfg.lp)+1/2*(p(cfg.lp)-p(cfg.lp-1));
phii=zeros( cfg.lphi+1,1);
phii(2: cfg.lphi)=1/2*(phi(1: cfg.lphi-1)+phi(2: cfg.lphi));
phii(1)=phi(1)-1/2*(phi(2)-phi(1));
phii( cfg.lphi+1)=phi( cfg.lphi)+1/2*(phi( cfg.lphi)-phi( cfg.lphi-1));


%SPHERICAL XFORMATION OF INTERFACES IN Q
rqi=zeros(cfg.lq+1,cfg.lp);
thetaqi=zeros(cfg.lq+1,cfg.lp);
for iq=1:cfg.lq+1
    for ip=1:cfg.lp
...
rqi=repmat(rqi,[1 1  cfg.lphi]);
thetaqi=repmat(thetaqi,[1 1  cfg.lphi]);


%SPHERICAL XFORMATION OF INTERFACES IN P
rpi=zeros(cfg.lq,cfg.lp+1);
thetapi=zeros(cfg.lq,cfg.lp+1);
for iq=1:cfg.lq
    for ip=1:cfg.lp+1
...
rpi=repmat(rpi,[1 1  cfg.lphi]);
thetapi=repmat(thetapi,[1 1  cfg.lphi]);
...
ephi(:,:,:,1)=-sin(phispher);
ephi(:,:,:,2)=cos(phispher);
ephi(:,:,:,3)=zeros(cfg.lq,cfg.lp, cfg.lphi);
...
xg.x1=q; xg.x2=p; xg.x3=reshape(phi,[1 1  cfg.lphi]);
xg.x1i=qi; xg.x2i=pii; xg.x3i=reshape(phii,[1 1  cfg.lphi+1]);
...
xg.gx1=gq; xg.gx2=gp; xg.gx3=zeros(cfg.lq,cfg.lp, cfg.lphi);
