function xgf=makegrid_tilteddipole_varx2_oneside_3D(dtheta,dphi,lpp,lqp,lphip,altmin,glat,glon,gridflag,lims)
...
if (exist('lims','var'))   % use user-provided limits
    pmin=lims(3);
    pmax=lims(4);
else
...
% KUMAMOTO
%coeffs=[8.664536621316147e-05, 2.050256907739271e-05, (9.689894149337605e-05)*2.04]; % Kumamoto
coeffs=[6.146167601073053e-09, -1.024081040872654e-08, (3.139765344578972e-04)*1.234]; % Kumamoto
...
if (exist('lims','var'))
    ...
    if gridflag==0
        thetamax=thetax2min+pi/15;        %open 15
    else
...
if gridflag==0
    ...
    amp=amp/2;
%    while q2(iq)>=-qmax
    while q2(iq)>=qmin    % don't force symmetric limits, use user-provided value
        iq=iq+1;
    ...
    % an issue here is that the last grid point *could* overstep a
    % user-provided qmax which is bad because then inteprolations will
    % produce NaNs here.  So remove the last point and assume the user will
